...
from src.utils.common import load_img_channel, load_img
...
class HPADataset(Dataset):
    def __init__(self, image_ids, label_map, path_data, path_masks, img_size=224):
        ...
        for image_id in image_ids:
            if image_id not in label_map:
                continue
            mask = load_img_channel(image_id, path_masks)
            if mask is None:
                continue
    ...
    def __getitem__(self, idx):
        ...
        img = load_img(image_id, self.path_data)
        mask = load_img_channel(image_id, self.path_masks)
